...
#ifdef SECURE
  BIGNUM peer_key;
  if (0 == (BN_hex2bn(&peer_key, &sm_pkt.pub_key))) {
    sm_pkt_udp_tx_st(sm_construct_resp(sm_pkt, SmErrType::kCryptoError));
    return;
  }
  if(NULL == (session->secret = OPENSSL_malloc(sizeof(unsigned char) * (DH_size(privkey))))) {
    assert(0); // FIXME
  }
  if(0 > (session->secret_size = DH_compute_key(session->secret, dh->pub_key, dh->priv_key))) {
    assert(0); // FIXME
  }
  if (0 == (BN_bn2hex(&sm_pkt.pub_key, dh->pub_key))) {
    sm_pkt_udp_tx_st(sm_construct_resp(sm_pkt, SmErrType::kCryptoError));
    return;
  }
#endif /* SECURE */
